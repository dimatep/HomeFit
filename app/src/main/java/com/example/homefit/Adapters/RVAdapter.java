...
import com.example.homefit.Activities.BuilderActivity;
import com.example.homefit.Activities.ExerciseActivity;
...
public class RVAdapter extends RecyclerView.Adapter<RVAdapter.WorkoutViewHolder> {
    ...
    @Override
    public WorkoutViewHolder onCreateViewHolder(ViewGroup parent, int viewType) { //layout inflater
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.programcard, parent, false);
        return new WorkoutViewHolder(v);
    }

    @Override
    public void onBindViewHolder(WorkoutViewHolder holder, final int position) { //set photos to workout types
        ...
        Glide.with(context).load(photo).into(holder.personPhoto);
    ...
    public void removeItem(int position) { //delete item
        workouts.remove(position);
        notifyItemRemoved(position);
    }

    public void restoreItem(Workout item, int position) { //if the use want to undo the action (delete)
