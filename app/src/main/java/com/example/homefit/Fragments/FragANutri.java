...
public class FragANutri extends Fragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        calc.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (ageT.getText().toString().trim().length() == 0 || ageT.getText().toString().equals("0") ||
                        ...
                        heightT.getText().toString().trim().length() == 0 || heightT.getText().toString().equals("0") ||
                        (weightT.getText().toString().length() > 0 && weightT.getText().toString().substring(0, 1).equals(".")))

                    Toast.makeText(getActivity(), "Please Make Sure Your Input is Correct", Toast.LENGTH_SHORT).show();
                else {
                    dialog.show();
                    if (gender.getDirection().name().equals("LEFT")) { //female
                    ...
                    } else if (gender.getDirection().name().equals("RIGHT")) { //male
                        if (measure.getDirection().name().equals("LEFT")) {
                    ...
                    int active = act.getCheckedTogglePosition();
                    int intense = act2.getCheckedTogglePosition();
                    int seekProgress = seek.getProgress();
                    tdee = tdeeTest(bmr, active, intense, seekProgress);
        ...
        reset.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) { //reset all the data
                ageT.setText("");
                heightT.setText("");
                weightT.setText("");
                gender.setDirection(StickySwitch.Direction.LEFT);
